...
export async function GET() {
  try {
    ...
    const [users, organizations, fundingRequests, transactions, files] = await Promise.all([
      // Users
    ...
    const activities = [
      ...users.map(user => ({
        id: `user-${user.id}`,
        type: 'user' as const,
        action: user.createdAt.getTime() === user.updatedAt.getTime() ? 'created' as const : 'updated' as const,
